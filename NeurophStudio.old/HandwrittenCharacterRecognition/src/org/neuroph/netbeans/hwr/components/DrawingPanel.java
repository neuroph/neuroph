...
public class DrawingPanel extends JPanel {
     ...
     */
    public BufferedImage getDrawnLetter() {
...
//        }
    ...
    public void saveDrawnLetter(String letter) {
        ...
        // extract letter image from the entire image
        BufferedImage subImage = reSizingImage(image.getSubimage(leftCoordinate, upperCoordinate,
                rightCoordinate - leftCoordinate, bottomCoordinate - upperCoordinate));
        // resize letter image to fixed size (normalize)
        BufferedImage resizedImage = ImageUtilities.blackAndLightGrayCleaning(subImage);
     
        int fileNumber = numberOfFiles(letter) + 1;
        try {
            ImageIO.write(resizedImage, "PNG", new File("Letters/Training Set/" + letter.toUpperCase() + "_" + fileNumber + ".png"));
        } catch (IOException ex) {
            throw new RuntimeException("No drawing could be founded, try writing again");
        }
    }
...
//        neurophImage = (ImageJ2SE) ocrUtil.resizeImage(neurophImage, WIDTH, WIDTH);
//
//        return ocrUtil.resizeImage(neurophImage, FIXED_WIDTH, FIXED_HEIGHT);
    }
