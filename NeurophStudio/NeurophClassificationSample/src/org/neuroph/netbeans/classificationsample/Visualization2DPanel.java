...
public class Visualization2DPanel extends javax.swing.JPanel implements ComponentListener {
  ...
  //  private int resolution;
    ...
    //returns boolean value that shows if the drawing is locked
    public boolean isDrawingLocked() {
        return drawingLocked;
    }

    //sets the drawingLocked(boolean) atribute
    ...
    //returns boolean value that shows if the visualization has started
    public boolean isVisualizationStarted() {
        return visualizationStarted;
    }

    //sets the visualization to started or not started
    ...
    //sets the boolean value pointDrawed
...
//    }
    ...
    private void initImageBuffer() {
            imageBuffer = createImage(panelSize, panelSize);//creates an off-screen drawable image to be used for double buffering
            graphicsBuffer = imageBuffer.getGraphics();//creates a graphics context for drawing to an off-screen image
            if (!positiveInputsOnly) {
