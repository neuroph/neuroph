...
//    public static ConvolutionLayer createConvolutionLayer(FeatureMapsLayer fromLayer, Kernel kernel) {
...
//        Layer2D.Dimension toDimension = new Layer2D.Dimension(toMapWidth, toMapHeight);
...
//    }
    ...
    /**
     * Creates and returns instance of Layer2D with specified dimension and neuron properties
     * @param mapDimension
     * @param neuronProperties
     * @return 
     */
//    public static Layer2D createFeatureMap(Layer2D.Dimension mapDimension, NeuronProperties neuronProperties) {
...
//    }
        ...
        /**
         * Creates next convolutional or pooling layer in convolutional network for given kernel
         * @param fromLayer
         * @param kernel
         * @param type
         * @return 
         * @deprecated 
         */
//	public static FeatureMapsLayer createNextLayer(FeatureMapsLayer fromLayer, Kernel kernel, Layer2DType type) {
//		Layer2D.Dimension fromDimension = fromLayer.getDimension();
...
//	}
        ...
        // umesto ove dve metode dodati jednu createFeatureMapsLayer(Dimension, howManyMaps,) - ovo mozda ima smisla i da se radi iz konstruktora
//	public static void addFeatureMap(FeatureMapsLayer mapsLayer) {
//		Layer2D.Dimension mapDimension = mapsLayer.getDimension();
//		Layer2D featureMap = new Layer2D(mapDimension);
//
//		for (int i = 0; i < mapDimension.getHeight() * mapDimension.getWidth(); i++) {
//			Neuron neuron = NeuronFactory.createNeuron(mapsLayer.getNeuronProperties());
//			featureMap.addNeuron(neuron);
//		}
...
//	public static void addFeatureMaps(FeatureMapsLayer mapsLayer, int numberOfLayers) {
//		for (int i = 0; i < numberOfLayers; i++) { // da li ovde treba isti layer da se doda nekoliko puta?
...
//	}
	...
	private static void connectPoolingLayer(Layer2D fromMap, Layer2D toMap, Kernel kernel) {
         ...
         * @param kernel 
         */
	private static void connectConvolutionLayerMaps(Layer2D fromMap, Layer2D toMap, Kernel kernel) {
...
//
//		for (int x = 0; x < fromMap.getWidth()-kernel.getWidth(); x++) { // upper limit changed -kernel.getWidth()
//			for (int y = 0; y < toMap.getHeight()-kernel.getHeight(); y++) { // -kernel.getHeight()
//				Neuron fromNeuron = fromMap.getNeuronAt(x, y);
//				for (int ky = 0; ky < kernel.getHeight(); ky++) {
//					for (int kx = 0; kx < kernel.getWidth(); kx++) {
//						int toX = x + kx;
//						int toY = y + ky;
//						int currentWeightIndex = kx + ky * kernel.getWidth(); // ovde je bilo getHeight
